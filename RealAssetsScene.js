...
function BusinessForm({ onSave, onCancel, T, editData, hideValues = false, properties = [] }) {
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      businessData: f,
      // Passive businesses: income in asset.income for passive tracker
      // Active businesses: income only in activeIncomes (via saveAsset sync below)
...
function RealAssetsScene({
  ...
  const removeAsset = (id) => {
    ...
    // Clean up activeIncomes if this was an active business
    if (setActiveIncomes) {
      setActiveIncomes(p => p.filter(a => a.id !== "biz_" + id));
    }
  };
